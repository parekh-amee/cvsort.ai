...
# Upload folder
app.config['UPLOAD_FOLDER'] = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'webapp', 'uploads'))
...
def predictResume(filename):
    try:
        ...
        labeldict = {
            0: 'Arts', 1: 'Automation Testing', 2: 'Operations Manager', 3: 'DotNet Developer',
            4: 'Civil Engineer', 5: 'Data Science', 6: 'Database', 7: 'DevOps Engineer',
            8: 'Business Analyst', 9: 'Health and fitness', 10: 'HR', 11: 'Electrical Engineering',
            12: 'Java Developer', 13: 'Mechanical Engineer', 14: 'Network Security Engineer',
            15: 'Blockchain ', 16: 'Python Developer', 17: 'Sales', 18: 'Testing', 19: 'Web Designing'
        }
...
def show_result():
    try:
        ...
        for resume in resumes:
            ...
            predicted = predictResume(resume_path)
            if predicted in my_profile:
                filtered_files.append(resume)

        jobdes = Preprocessfile(my_jd)
        customKeywords = [tag.strip() for tag in my_tags]

        res = []
        for file in filtered_files:
            resume_path = os.path.join(app.config['UPLOAD_FOLDER'], user_id, file)
            score = find_score(jobdes, resume_path, customKeywords)
            user_info = ResumeParser(resume_path).get_extracted_data()
            user_info['predicted'] = predictResume(resume_path)
